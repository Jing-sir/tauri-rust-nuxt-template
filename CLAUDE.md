...
>
> This file is the repository-level instruction set for Claude Code.
> 本文件是提供给 Claude Code 的仓库级规则说明。
...
- CN: `src/utils/dayjs.ts` 当前看起来像 Nuxt 配置片段，不是主配置入口；如需调整 dayjs 行为，应优先检查 `nuxt.config.ts` 的 `dayjs-nuxt` 模块配置。
- EN: `src/utils/dayjs.ts` currently looks like a stray Nuxt config fragment rather than the real entrypoint; if you need to change Dayjs behavior, inspect the `dayjs-nuxt` setup in `nuxt.config.ts` first.
