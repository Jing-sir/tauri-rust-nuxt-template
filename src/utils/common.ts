...
export const getGlobalI18nText = (val: string): string => useNuxtApp().$i18n.t(val);

export function numberToChinese(numberInput: number): string {
    const chineseNumbers = ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九'] as const;
    const chineseUnits = ['', '十', '百', '千', '万', '十万', '百万', '千万', '亿'] as const;
    ...
    while (number > 0) {
        ...
        if (digit !== 0) {
            const digitText = chineseNumbers[digit] ?? '';
            const unitText = chineseUnits[unitIndex] ?? '';
            if (zeroFlag) {
                chineseStr = chineseNumbers[0] + chineseStr;
                zeroFlag = false;
            }
            chineseStr = digitText + unitText + chineseStr;
        } else {
            zeroFlag = true;
        }
...
export const handlePaste = async (): Promise<string[]> => {
    ...
    if (!window.navigator.clipboard) {
        message.warning('该浏览器不支持粘贴功能或需要进行授权操作');
        // 该浏览器不支持粘贴功能或需要进行授权操作
        throw new Error('该浏览器不支持粘贴功能'); // 该浏览器不支持粘贴功能
    }

    try {
        const text = await window.navigator.clipboard.readText();
        if (!SIX_NUMBER.test(text)) {
            message.error('请粘贴6位数字');
            return [];
        }
...
export function toCapital(n: number | string): string {
    ...
    const s: string = Array.from(numString).reduce(
        (acc, cur) => acc + (cNum[Number(cur)] ?? ''),
        '',
    );

    if (s.length !== 2) return s; // 非两位数的时候

    const zeroText = cNum[0] ?? '';
    const oneText = cNum[1] ?? '';
    const tenText = cNum[10] ?? '';

    if (s.charAt(1) === zeroText) {
        // 如果个位数是0的时候，令改成十
        return s === oneText + tenText /* 如果是一十改成十 */ ? tenText : s.charAt(0) + tenText;
    }

    if (s.charAt(0) === oneText) return tenText + s.charAt(1);
